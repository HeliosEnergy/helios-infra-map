...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const result = await setAccessRequestStatus({ token, status, decidedBy: 'email-link' });
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    ...
    const message =
      status === 'approved'
        ? `Request for ${result.request?.email || 'user'} has been approved. They can now create an account with that email.`
        : `Request for ${result.request?.email || 'user'} has been rejected.`;

    return res.status(200).send(page(status === 'approved' ? 'Access Approved' : 'Access Rejected', message));
  } catch (error) {
