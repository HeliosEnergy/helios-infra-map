...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // GET — check if an existing token is still valid
  if (req.method === 'GET') {
    if (!isAuthConfigured()) {
  ...
  // POST — simple password login (no Supabase needed)
  if (req.method !== 'POST') {
  ...
  if (!isAuthConfigured()) {
    return res.status(503).json({
      error: 'Server auth is not configured. Set APP_PASSWORD and AUTH_JWT_SECRET in .env.',
    });
  }
